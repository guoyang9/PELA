...
def low_rank_approximate(mat_org: torch.tensor, rank=32):
    ...
    mat_l = torch.tensor(u[:, :rank] @ s_val)
    mat_r = torch.tensor(s_val @ vh[:rank, :])
    error = nn.functional.mse_loss(mat_l @ mat_r, mat_org)
...
class ModuleLowRank(object):
    ...
    def _apply(self, name: str, module: nn.Linear):
        ...
        if self.is_approximate:
            ...
            weight_l, weight_r, error = lr_out['mat_l'], lr_out['mat_r'], lr_out['error']
            ...
            weight_l, weight_r, error = None, None, None
    ...
    def __call__(self, module: nn.Module):
        ...
        for name, module_sub in module.named_modules():
            if isinstance(module_sub, nn.Linear):
                ...
                # module_sub.weight.data.copy_((output['weight_r'] @ output['weight_l'].t()))
