...
class ALBEF(nn.Module):
    def __init__(self,
        ...
        self.visual_encoder = VisionTransformer(
            ...
            mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), output_hidden_states=True)
    ...
    def forward(self, image, text, alpha=0, model_t=None):
        with torch.no_grad():
            self.temp.clamp_(0.001,0.5)

        image_embeds, image_outputs = self.visual_encoder(image)
        ...
        loss_norm = inf_norm(model_t, self) if model_t else None
        output = {
            'image_out': image_outputs,
            'text_out': text_output.hidden_states,
            'text_mlm_out': mlm_output.hidden_states,
        }

        return loss_mlm, loss_ita, loss_itm, loss_norm, output
...
def inf_norm(model_t, model_s, eps=1e-3):
    ...
    for name_t, param_t in model_t.named_parameters():
        if 'weight' in name_t: # only compress linear layer with weight
            ...
            if name_s1 in model_s_dict and name_s2 in model_s_dict:
                ...
                residual = torch.norm((rebuild - param_t), float('inf'))

                # TODO: more efficient inf norm
                error = F.mse_loss(residual, target)
                norm_error += error
