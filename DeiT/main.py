...
from engine import train_one_epoch, evaluate, evaluate_feature
...
def get_args_parser():
    ...
    parser.add_argument('--inf_norm_eps', default=1e-3, type=float)
    parser.add_argument('--inf_norm_weight', default=0.0, type=float)
    ...
    parser.add_argument('--scale_lr', default=0, type=int) # scale learning rate according to batch size
...
def main(args):
    if args.scale_lr:
        args.lr = args.batch_size * args.lr / 256 # 256 is preset batch size
    ...
    if args.distributed:
        if args.distillation:
            model_t = torch.nn.parallel.DistributedDataParallel(model_t, device_ids=[args.gpu], broadcast_buffers=False)
    ...
    if args.eval:
        test_stats = evaluate_feature(data_loader_val, model, device)
